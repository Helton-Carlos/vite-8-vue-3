...
const normalCount = computed(
  () => records.value.filter((r) => r.status === 'normal').length,
);
const alertCount = computed(
  () => records.value.filter((r) => r.status === 'alerta').length,
);
const criticalCount = computed(
  () => records.value.filter((r) => r.status === 'crítico').length,
);
...
function saveRecord() {
  if (!form.value.tankId) {
    toast.error('Selecione um tanque', {
  ...
  if (status === 'crítico') {
    toast.error(`Atenção: parâmetros críticos no ${tank?.name}`, {
      position: toast.POSITION.BOTTOM_RIGHT,
    });
  } else {
      ...
      <div class="card bg-base-200 shadow-sm hover:shadow-md transition-shadow">
        <div class="card-body">
          ...
          <div class="flex gap-4 text-xs opacity-60 mt-1">
            <span class="flex items-center gap-1"><span class="w-3 h-0.5 bg-current inline-block" />
              Temperatura</span>
            <span class="flex items-center gap-1"><span class="w-3 h-0.5 bg-success inline-block" /> pH</span>
          </div>
        </div>
      </div>
      <div class="card bg-base-200 shadow-sm hover:shadow-md transition-shadow">
    ...
    >
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">Temperatura (°C)</span></label>
          <input
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">pH</span></label>
          <input
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">O₂ Dissolvido (mg/L)</span></label>
          <input
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">Amônia (mg/L)</span></label>
          <input
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">Nitrito (mg/L)</span></label>
          <input
        ...
        <div class="form-control">
          <label class="label"><span class="label-text text-xs font-medium">Transparência (cm)</span></label>
          <input
